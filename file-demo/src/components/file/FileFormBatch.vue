<template>
  <el-form ref="form" :model="form" label-width="110px">
    ...
    <el-button style="margin-left: 10px;" type="primary" @click="submitUpload">批量上传到服务器</el-button>
  </el-form>
</template>
<script>
...
export default {
  ...
  methods: {
    submitUpload() {
      if (!this.selectedFileList || this.selectedFileList.size < 1) {
        alert('请选择您要上传的图片~');
        return false;
      }
      ...
      for(let i = 0; i < this.selectedFileList.length; i++) {
        formData.append('fileList[' + i + ']', this.selectedFileList[i]);
      }
      //formData.append('fileList', this.selectedFileList);
      server.post('/api/common/multiple/file/uploadBatch', formData, {
        headers: {
    ...
    handleChange(file, fileList) {
      let fileTempList = [];
      for (const fileListKey of fileList) {
        fileTempList.push(fileListKey.raw)
      }
      this.selectedFileList = fileTempList;
    },
